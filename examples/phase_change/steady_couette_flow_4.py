r"""
Based on the paper Laminar Heat Transfer to a Steady Couette Flow
between parallel Plates.

A parallel plate channel is formed by a stationary lower wall and a 
uniformly moving upper plate. Assuming temperature independent thermophysical
properties of the transported fluid, the upper plate induces a uniform
velocity gradient in the channel - :math:`\frac{U}{H} y`.
Longitudinal cross section is taken as the domain.

CASE D:

:math:`T _0 for x \leqslant 0, 0 \geqslant y \leqslant H`
:math:`\fraction {\partial T}{\partial y} = 0 for x > 0, y=0`
:math:`T _1 for x > 0, y=H`
...
filename_mesh = data_dir + '/meshes/2d/rectangle_fine_tri.mesh'

regions = {
    'Omega' : ('all', {}),
    'Gamma_Wall' : ('nodes in (x < -4.99999)', {}),
    'Gamma_Plate' : ('nodes in (x > 4.99999)', {}),
    'Gamma_Entry' : ('nodes in (y > 9.99999)', {}),
    'Omega_L' : ('r.Omega -n ( r.Gamma_Wall +n r.Gamma_Plate +n r.Gamma_Entry )', {}),
}
...
ebcs = {
    'T1' : ('Gamma_Plate', {'T.0' : 40}),
    'T2' : ('Gamma_Entry', {'T.0' : 20}),
}
...
def get_convective_velocity(ts, coors, region=None):
    ...
    """
    val = nm.ones_like(coors)
    val[:,1] = -1.0*((coors[:,0]+5.0)/10.0) * val[:,1]
    val[:,0] = 0
    return val
