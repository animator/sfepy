...
class PhaseChangeSolver(Solver):
    ...
    """
        ...
        self.YCVRS[M2 - 1] = 0.5 * (self.R[M1 - 1] + self.R[M3 - 1]) \
            * self.YCVS[M2 - 1]
        ...
        for J in range(2, M3):
            ...
            self.FVP[J] = 1. - self.FV[J]
        for I in range(2, L2):
            self.FX[I] = 0.5 * self.XCV[I - 1] / self.XDIF[I]
            self.FXM[I] = 1. - self.FX[I]
        self.FX[1] = 0.
        self.FXM[1] = 1.
        self.FX[L1 - 1] = 1.
        self.FXM[L1 - 1] = 0.
        ...
        if self.NF <= 1:
            for J in range(0, self.M1):
                for I in range(0, self.L1):
                    ...
                    if self.NF == 1:
                        self.GAM[I, self.M1 - 1] = 0.  # top face
                        TMEAN = self.FY[J] * self.T[I, J] + self.FYM[J] \
                            * self.T[I, J - 1]
                        self.CON[I, J] = self.RHOREF * self.G * self.BETA \
                            * (TMEAN - self.TMELT)
        ...
        if self.NF == 3:
            for J in range(0, self.M1):
                for I in range(0, self.L1):
                    ...
                    self.CON[I, J] = -1. * self.RHO[I, J] * (self.DELH[I, J]
                            - self.DELHO[I, J]) / (self.CP * self.DT)
        ...
        for NT in range(0, self.NTIMES[self.NF]):
            if self.LBLK[self.NF]:
                ...
                for I in range(self.IST - 1, self.L2):
                    ...
                    for J in range(self.JST - 1, self.M2):
                        ...
                        BLC = BLC + self.CON[I, J] + self.AIP[I, J] * self.F[I
                                + 1, J] + self.AIM[I, J] * self.F[I - 1, J] \
                            + self.AJP[I, J] * self.F[I, J + 1] + self.AJM[I,
                                J] * self.F[I, J - 1] - self.AP[I, J] \
                            * self.F[I, J]
            ...
            for J in range(self.JST - 1, self.M2):
                ...
                for I in range(self.IST - 1, self.L2):
                    ...
                    self.TEMP = self.CON[I, J] + self.AJP[I, J] * self.F[I, J
                            + 1] + self.AJM[I, J] * self.F[I, J - 1]
                    self.QT[I] = (self.TEMP + self.AIM[I, J] * self.QT[I - 1]) \
                        / DENOM
                for II in range(self.IST - 1, self.L2):
                    I = IT1 - II
            ...
            for I in range(self.IST - 1, self.L2):
                ...
                for J in range(self.JST - 1, self.M2):
                    ...
                    self.TEMP = self.CON[I, J] + self.AIP[I, J] * self.F[I
                            + 1, J] + self.AIM[I, J] * self.F[I - 1, J]
                    self.QT[J] = (self.TEMP + self.AJM[I, J] * self.QT[J - 1]) \
                        / DENOM
                for JJ in range(self.JST - 1, self.M2):
                    J = JT1 - JJ
    ...
    # COEFFICIENTS FOR THE U EQUATION.
        ...
        while self.LCONV == 0:
            ...
            if self.LSOLVE[self.NF]:
                ...
                self.JST = 2
                self.gamsor()
                self.REL = 1. - self.RELAX[self.NF]
                for I in range(2, self.L2):
                    FL = self.XCVI[I] * self.V[I, 1] * self.RHO[I, 0]
                    FLM = self.XCVIP[I - 1] * self.V[I - 1, 1] * self.RHO[I
                            - 1, 0]
                    self.FLOW = self.R[0] * (FL + FLM)
                    self.DIFF = self.R[0] * (self.XCVI[I] * self.GAM[I, 0]
                            + self.XCVIP[I - 1] * self.GAM[I - 1, 0]) \
                        / self.YDIF[1]
                    self.diflow()
                ...
                for J in range(1, self.M2):
                    ...
                    self.DIFF = self.ARX[J] * self.GAM[0, J] / (self.XCV[1]
                            * self.SX[J])
                    self.diflow()
                    self.AIM[2, J] = self.ACOF + max(self.ZERO, self.FLOW)
                    for I in range(2, self.L2):
                        if I != self.L2 - 1:
                            FL = self.U[I, J] * (self.FX[I] * self.RHO[I, J]
                                    + self.FXM[I] * self.RHO[I - 1, J])
                            FLP = self.U[I + 1, J] * (self.FX[I + 1]
                                    * self.RHO[I + 1, J] + self.FXM[I + 1]
                                    * self.RHO[I, J])
                            self.FLOW = self.ARX[J] * 0.5 * (FL + FLP)
                            self.DIFF = self.ARX[J] * self.GAM[I, J] \
                                / (self.XCV[I] * self.SX[J])
                        else:
                            self.FLOW = self.ARX[J] * self.U[self.L1 - 1, J] \
                                * self.RHO[self.L1 - 1, J]
                            self.DIFF = self.ARX[J] * self.GAM[self.L1 - 1, J] \
                                / (self.XCV[self.L2 - 1] * self.SX[J])
                        self.diflow()
                        self.AIM[I + 1, J] = self.ACOF + max(self.ZERO,
                                self.FLOW)
                        self.AIP[I, J] = self.AIM[I + 1, J] - self.FLOW
                        if J != self.M2 - 1:
                            FL = self.XCVI[I] * self.V[I, J + 1] * (self.FY[J
                                    + 1] * self.RHO[I, J + 1] + self.FYM[J
                                    + 1] * self.RHO[I, J])
                            FLM = self.XCVIP[I - 1] * self.V[I - 1, J + 1] \
                                * (self.FY[J + 1] * self.RHO[I - 1, J + 1]
                                   + self.FYM[J + 1] * self.RHO[I - 1, J])
                            GM = self.GAM[I, J] * self.GAM[I, J + 1] \
                                / (self.YCV[J] * self.GAM[I, J + 1]
                                   + self.YCV[J + 1] * self.GAM[I, J] + 10.0
                                   ** -30) * self.XCVI[I]
                            GMM = self.GAM[I - 1, J] * self.GAM[I - 1, J + 1] \
                                / (self.YCV[J] * self.GAM[I - 1, J + 1]
                                   + self.YCV[J + 1] * self.GAM[I - 1, J]
                                   + 10.0 ** -30) * self.XCVIP[I - 1]
                            self.DIFF = self.RMN[J + 1] * 2.0 * (GM + GMM)
                        else:
                            FL = self.XCVI[I] * self.V[I, self.M1 - 1] \
                                * self.RHO[I, self.M1 - 1]
                            FLM = self.XCVIP[I - 1] * self.V[I - 1, self.M1
                                    - 1] * self.RHO[I - 1, self.M1 - 1]
                            self.DIFF = self.R[self.M1 - 1] * (self.XCVI[I]
                                    * self.GAM[I, self.M1 - 1] + self.XCVIP[I
                                    - 1] * self.GAM[I - 1, self.M1 - 1]) \
                                / self.YDIF[self.M1 - 1]
                        self.FLOW = self.RMN[J + 1] * (FL + FLM)
                        self.diflow()
                        self.AJM[I, J + 1] = self.ACOF + max(self.ZERO,
                                self.FLOW)
                        self.AJP[I, J] = self.AJM[I, J + 1] - self.FLOW
                        self.VOL = self.YCVR[J] * self.XCVS[I]
                        self.APT = (self.RHO[I, J] * self.XCVI[I] + self.RHO[I
                                    - 1, J] * self.XCVIP[I - 1]) \
                            / (self.XCVS[I] * self.DT)
                        self.AP[I, J] = self.AP[I, J] - self.APT
                        self.CON[I, J] = self.CON[I, J] + self.APT \
                            * self.UO[I, J]
                        self.AP[I, J] = (-self.AP[I, J] * self.VOL
                                + self.AIP[I, J] + self.AIM[I, J]
                                + self.AJP[I, J] + self.AJM[I, J]) \
                            / self.RELAX[self.NF]

            # CARMAN-KOZENY RELATION FOR POROUS MEDIA  EFFECTS
                        self.AP0[I, J] = self.RHO[I, J] * self.VOL / self.DT
                        EPMEAN = self.FX[I] * self.EPSI[I, J] + self.FXM[I] \
                            * self.EPSI[I - 1, J]
                        self.AP[I, J] = self.AP[I, J] + 1. * self.DARCYCONST \
                            * (1. - EPMEAN) ** 2 * self.VOL / (EPMEAN ** 3
                                + 10.0 ** -3)
                        self.CON[I, J] = self.CON[I, J] * self.VOL + self.REL \
                            * self.AP[I, J] * self.U[I, J]
        ...
        # COEFFICIENTS FOR THE  V  EQUATION.
                ...
                for J in range(2, self.M2):
                    ...
                    FLM = self.ARXJP[J - 1] * self.U[1, J - 1] * self.RHO[0, J
                            - 1]
                    self.FLOW = FL + FLM
                    self.DIFF = (self.ARXJ[J] * self.GAM[0, J] + self.ARXJP[J
                                 - 1] * self.GAM[0, J - 1]) / (self.XDIF[1]
                            * self.SXMN[J])
                    self.diflow()
                    self.AIM[1, J] = self.ACOF + max(self.ZERO, self.FLOW)
                    for I in range(1, self.L2):
                        if I != self.L2 - 1:
                            FL = self.ARXJ[J] * self.U[I + 1, J] * (self.FX[I
                                    + 1] * self.RHO[I + 1, J] + self.FXM[I
                                    + 1] * self.RHO[I, J])
                            FLM = self.ARXJP[J - 1] * self.U[I + 1, J - 1] \
                                * (self.FX[I + 1] * self.RHO[I + 1, J - 1]
                                   + self.FXM[I + 1] * self.RHO[I, J - 1])
                            GM = self.GAM[I, J] * self.GAM[I + 1, J] \
                                / (self.XCV[I] * self.GAM[I + 1, J]
                                   + self.XCV[I + 1] * self.GAM[I, J] + 10.0
                                   ** -30) * self.ARXJ[J]
                            GMM = self.GAM[I, J - 1] * self.GAM[I + 1, J - 1] \
                                / (self.XCV[I] * self.GAM[I + 1, J - 1]
                                   + self.XCV[I + 1] * self.GAM[I, J - 1]
                                   + 10.0 ** -30) * self.ARXJP[J - 1]
                            self.DIFF = 2.0 * (GM + GMM) / self.SXMN[J]
                        else:
                            FL = self.ARXJ[J] * self.U[self.L1 - 1, J] \
                                * self.RHO[self.L1 - 1, J]
                            FLM = self.ARXJP[J - 1] * self.U[self.L1 - 1, J
                                    - 1] * self.RHO[self.L1 - 1, J - 1]
                            self.DIFF = (self.ARXJ[J] * self.GAM[self.L1 - 1,
                                    J] + self.ARXJP[J - 1] * self.GAM[self.L1
                                    - 1, J - 1]) / (self.XDIF[self.L1 - 1]
                                    * self.SXMN[J])
                        self.FLOW = FL + FLM
                        self.diflow()
                        self.AIM[I + 1, J] = self.ACOF + max(self.ZERO,
                        ...
                        if J != self.M2 - 1:
                            ...
                            FL = self.V[I, J] * (self.FY[J] * self.RHO[I, J]
                                    + self.FYM[J] * self.RHO[I, J - 1]) \
                                * self.RMN[J]
                            FLP = self.V[I, J + 1] * (self.FY[J + 1]
                                    * self.RHO[I, J + 1] + self.FYM[J + 1]
                                    * self.RHO[I, J]) * self.RMN[J + 1]
                            self.FLOW = (self.FV[J] * FL + self.FVP[J] * FLP) \
                                * self.XCV[I]
                            self.DIFF = self.AREA * self.GAM[I, J] \
                                / self.YCV[J]
                        else:
                            self.AREA = self.R[self.M1 - 1] * self.XCV[I]
                            self.FLOW = self.AREA * self.V[I, self.M1 - 1] \
                                * self.RHO[I, self.M1 - 1]
                            self.DIFF = self.AREA * self.GAM[I, self.M1 - 1] \
                                / self.YCV[self.M2 - 1]
                        self.diflow()
                        self.AJM[I, J + 1] = self.ACOF + max(self.ZERO,
                                self.FLOW)
                        ...
                        if J == 2:
                            SXB = self.SX[0]
                        self.APT = (self.ARXJ[J] * self.RHO[I, J] * 0.5 * (SXT
                                    + self.SXMN[J]) + self.ARXJP[J - 1]
                                    * self.RHO[I, J - 1] * 0.5 * (SXB
                                    + self.SXMN[J])) / (self.YCVRS[J]
                                * self.DT)
                        self.AP[I, J] = self.AP[I, J] - self.APT
                        self.CON[I, J] = self.CON[I, J] + self.APT \
                            * self.VO[I, J]
            ...
            # THIS IS CORMAN RELATION FOR POROUS MEDIA EFFECTS
                        self.AP0[I, J] = self.RHO[I, J] * self.VOL / self.DT
                        EPMEAN = self.FY[J] * self.EPSI[I, J] + self.FYM[J] \
                            * self.EPSI[I, J - 1]
                        self.AP[I, J] = self.AP[I, J] + 1. * self.DARCYCONST \
                            * (1. - EPMEAN) ** 2 * self.VOL / (EPMEAN ** 3
                                + 10.0 ** -3)
                        self.CON[I, J] = self.CON[I, J] * self.VOL + self.REL \
                            * self.AP[I, J] * self.V[I, J]
        ...
        # CALCULATE UHAT AND VHAT.
                for J in range(1, self.M2):
                    for I in range(2, self.L2):
                        self.UHAT[I, J] = (self.COFU[I, J, 1] * self.U[I + 1,
                                J] + self.COFU[I, J, 2] * self.U[I - 1, J]
                                + self.COFU[I, J, 3] * self.U[I, J + 1]
                                + self.COFU[I, J, 4] * self.U[I, J - 1]
                                + self.COFU[I, J, 0]) / self.COFU[I, J, 5]
                for J in range(2, self.M2):
                    for I in range(1, self.L2):
                        self.VHAT[I, J] = (self.COFV[I, J, 1] * self.V[I + 1,
                                J] + self.COFV[I, J, 2] * self.V[I - 1, J]
        ...
        # COEFFICIENTS FOR THE PRESSURE EQUATION.
                ...
                for J in range(1, self.M2):
                    ...
                    for I in range(1, self.L2):
                        if I != self.L2 - 1:
                            ARHO = self.ARX[J] * (self.FX[I + 1] * self.RHO[I
                                    + 1, J] + self.FXM[I + 1] * self.RHO[I, J])
                            ...
                            self.CON[I, J] = self.CON[I, J] - ARHO \
                                * self.U[self.L1 - 1, J]
                            self.AIP[I, J] = 0.
                        if J != self.M2 - 1:
                            ARHO = self.RMN[J + 1] * self.XCV[I] * (self.FY[J
                                    + 1] * self.RHO[I, J + 1] + self.FYM[J
                                    + 1] * self.RHO[I, J])
                            ...
                            ARHO = self.RMN[self.M1 - 1] * self.XCV[I] \
                                * self.RHO[I, self.M1 - 1]
                            self.CON[I, J] = self.CON[I, J] - ARHO * self.V[I,
                                    self.M1 - 1]
                            self.AJP[I, J] = 0.
                        self.AP[I, J] = self.AIP[I, J] + self.AIM[I, J] \
                            + self.AJP[I, J] + self.AJM[I, J]

                for J in range(1, self.M2):
                    for I in range(1, self.L2):
                        self.AP[I, J] = self.AP[I, J] / self.RELAX[self.NP]
                        self.CON[I, J] = self.CON[I, J] + (1.
                                - self.RELAX[self.NP]) * self.AP[I, J] \
                            * self.P[I, J]
        ...
        # COMPUTE U* AND V*.
                ...
                for J in range(self.JST - 1, self.M2):
                    for I in range(self.IST - 1, self.L2):
                        self.CON[I, J] = self.CON[I, J] + self.DU[I, J] \
                            * self.AP[I, J] * (self.P[I - 1, J] - self.P[I, J])
                self.solve()
                ...
                for J in range(self.JST - 1, self.M2):
                    for I in range(self.IST - 1, self.L2):
                        self.CON[I, J] = self.CON[I, J] + self.DV[I, J] \
                            * self.AP[I, J] * (self.P[I, J - 1] - self.P[I, J])
                self.solve()
        ...
        # COME HERE TO CORRECT THE VELOCITIES.
                for J in range(1, self.M2):
                    for I in range(1, self.L2):
                        if I != 1:
                            self.U[I, J] = self.U[I, J] + self.DU[I, J] \
                                * (self.PC[I - 1, J] - self.PC[I, J])
                        if J != 1:
                            self.V[I, J] = self.V[I, J] + self.DV[I, J] \
                                * (self.PC[I, J - 1] - self.PC[I, J])
      ...
      # COEFFICIENTS FOR OTHER EQUATIONS.
            ...
            for N in range(3, self.NFMAX):
                ...
                if self.LSOLVE[self.NF]:
                    ...
                    for J in range(1, self.M2):
                        self.FLOW = self.ARX[J] * self.U[1, J] * self.RHO[0, J]
                        self.DIFF = self.ARX[J] * self.GAM[0, J] \
                            / (self.XDIF[1] * self.SX[J])
                        self.diflow()
                        self.AIM[1, J] = self.ACOF + max(self.ZERO, self.FLOW)
                        for I in range(1, self.L2):
                            if I != self.L2 - 1:
                                self.FLOW = self.ARX[J] * self.U[I + 1, J] \
                                    * (self.FX[I + 1] * self.RHO[I + 1, J]
                                       + self.FXM[I + 1] * self.RHO[I, J])
                                self.DIFF = self.ARX[J] * 2.0 * self.GAM[I, J] \
                                    * self.GAM[I + 1, J] / ((self.XCV[I]
                                        * self.GAM[I + 1, J] + self.XCV[I + 1]
                                        * self.GAM[I, J] + 10.0 ** -30)
                                        * self.SX[J])
                            else:
                                self.FLOW = self.ARX[J] * self.U[self.L1 - 1,
                                        J] * self.RHO[self.L1 - 1, J]
                                self.DIFF = self.ARX[J] * self.GAM[self.L1
                                        - 1, J] / (self.XDIF[self.L1 - 1]
                                        * self.SX[J])
                            self.diflow()
                            self.AIM[I + 1, J] = self.ACOF + max(self.ZERO,
                                    self.FLOW)
                            ...
                            if J != self.M2 - 1:
                                self.FLOW = self.AREA * self.V[I, J + 1] \
                                    * (self.FY[J + 1] * self.RHO[I, J + 1]
                                       + self.FYM[J + 1] * self.RHO[I, J])
                                self.DIFF = self.AREA * 2.0 * self.GAM[I, J] \
                                    * self.GAM[I, J + 1] / (self.YCV[J]
                                        * self.GAM[I, J + 1] + self.YCV[J + 1]
                                        * self.GAM[I, J] + 10.0 ** -30)
                            else:
                                self.FLOW = self.AREA * self.V[I, self.M1 - 1] \
                                    * self.RHO[I, self.M1 - 1]
                                self.DIFF = self.AREA * self.GAM[I, self.M1
                                        - 1] / self.YDIF[self.M1 - 1]
                            self.diflow()
                            self.AJM[I, J + 1] = self.ACOF + max(self.ZERO,
                                    self.FLOW)
                            self.AJP[I, J] = self.AJM[I, J + 1] - self.FLOW
                    ...
                    for I in range(0, self.L1):
                        for J in range(0, self.M1):
                            ...
                            if self.NF == 3:
                                self.CON[I, J] = self.CON[I, J] * self.VOL \
                                    + self.REL * self.AP[I, J] * self.T[I, J]
                            if self.NF == self.NP - 1:
                                self.CON[I, J] = self.CON[I, J] * self.VOL \
                                    + self.REL * self.AP[I, J] * self.P[I, J]
                            if self.NF == 3:
                                self.TOLD[I, J] = self.T[I, J]
                    self.solve()
          ...
          # THIS IS FOR ENTHALPY UPDATING
                    for J in range(1, self.M2):
                        for I in range(1, self.L2):
                            self.VOL = self.YCVR[J] * self.XCV[I]
                            self.AP0[I, J] = self.RHO[I, J] * self.VOL \
                                / self.DT
                            self.DELH[I, J] = self.DELH[I, J] + self.AP1[I, J] \
                                * self.CP * self.ARELAX * (self.T[I, J]
                                    - self.TMELT) / self.AP0[I, J]
    ...
    """
        ...
        if field == 'u':
            for I in range(0, self.L1):
                for J in range(0, self.M1):
                    self.var_dict[field].data[I * self.NJ + J] = [self.Ur[I,
                            J], self.Vr[I, J]]
        ...
        if field == 'epsi':
            for I in range(0, self.L1):
                for J in range(0, self.M1):
                    self.var_dict[field].data[I * self.NJ + J] = self.EPSI[I,
                            J]
            return self.var_dict[field]
    ...
    """
        ...
        for I in range(0, self.L1 - 1):
            for J in range(0, self.M1 - 1):
                conns[I * (self.NJ - 1) + J] = [(I + 1) * self.NJ + J + 1, I
                        * self.NJ + J + 1, I * self.NJ + J, (I + 1) * self.NJ
                        + J]
